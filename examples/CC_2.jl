...
begin
  sys_discrete_1 = c2d(benchmarks[:CC], Tₛ)

  global dₜ_optimal = 0
  global max_total_reward_1 = -999
  for dₜ = 0:0.001:1
	global max_total_reward_1
	global dₜ_optimal
	env = Environment(sys_discrete_1)
	...
	if max_total_reward_1 < total_reward
	  max_total_reward_1 = total_reward
	  dₜ_optimal = dₜ
	end
  end
...
println(dₜ_optimal, " ", max_total_reward_1)

# ╔═╡ 76773064-937c-4145-bafd-248287a8954b
begin
  sys_discrete_2 = c2d(benchmarks[:CC], Tₛ)

  α_optimal = 0
  max_total_reward_2 = -999
  for α = 0.2:0.001:0.6
	env = Environment(sys_discrete_2)
    actions, states, distances, utilizations, ideal_states = sim!(env, π, dₜ_optimal, H)
	discounted_rewards, total_reward = reward(distances, utilizations, α, γ, H)

	if max_total_reward_2 < total_reward
	  max_total_reward_2 = total_reward
	  α_optimal = α
	end
  end
...
# ╔═╡ 6e59a013-1c02-4bbf-8f4d-f9b345aecb42
println(α_optimal, " ", max_total_reward_2)

# ╔═╡ 5f1a5b4e-af9c-42eb-8510-19e1a349fb51
begin
  sys_discrete_3 = c2d(benchmarks[:CC], Tₛ)

  γ_optimal = 0
  max_total_reward_3 = -999
  for γ = 0.9:0.001:1
	env = Environment(sys_discrete_3)
    actions, states, distances, utilizations, ideal_states = sim!(env, π, dₜ_optimal, H)
	discounted_rewards, total_reward = reward(distances, utilizations, α, γ, H)

	if max_total_reward_3 < total_reward
	  max_total_reward_3 = total_reward
	  γ_optimal = γ
	end
  end
...
# ╔═╡ 53c830cd-9c15-4a08-a4ad-03fc2b4bca4d
println(γ_optimal, " ", max_total_reward_3)

# ╔═╡ 50a726a3-9478-4e4d-a059-9457a3818210
begin
  env = Environment(c2d(benchmarks[:CC], Tₛ))
  actions, states, distances, utilizations, ideal_states = sim!(env, π, dₜ_optimal, H)
  discounted_rewards, total_reward = reward(distances, utilizations, α, γ, H)
end
